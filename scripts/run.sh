...
i=1
JOBS=""
for key in $APP/scripts/gnupg.{u*,c*,a*,b*}; do
...
# send the "joining" message to each server
KV_ADDR=6001
while [ $KV_ADDR -lt $API_ADDR ]; do
    curl http://localhost:$KV_ADDR/joining
    KV_ADDR=`expr $KV_ADDR + 1`
done
