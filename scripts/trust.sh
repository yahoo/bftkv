...
for i in "$@"; do
    id=`basename $i`
    $GPG --homedir .$i --export $id | sign.sh -o /tmp/$id.gpg $signer
...
for i in "$@"; do
	...
	$GPG --homedir .$signer --batch --no-tty --fast-import /tmp/$id.gpg > /dev/null 2>&1
	if [ $? -ne 0 ]; then echo "$0: failed to import $i"; fi
    fi
    if [ "$trust" == "both" ] || [ "$trust" == "signee" ]; then
	$GPG --homedir .$i --batch --no-tty --fast-import /tmp/$id.gpg > /dev/null 2>&1
	if [ $? -ne 0 ]; then echo "$0: failed to import $i"; fi
...
if [ "$trust" == "both" ] || [ "$trust" == "signer" ]; then
    $GPG --homedir .$signer --batch --no-tty --export > $signer/pubring.gpg
fi
