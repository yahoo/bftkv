...
\]
Each ``share'' is $(i, f(i)), i = 1..n$. To reconstruct the shared secret $f(0)$,
calculate lagrange interpolate from $t$ servers out of $n$
\[
...
\[ S = M^{f(0)} \bmod N \]
and combine partial signatures
\[ \prod_{i \in \mathcal{K}} M^{f(i)\lambda_i} \]
...
that may make it difficult to apply existing keys to the
...
\cite[Delhi:1,Delhi:2], which uses signed messages and
...
\begin{algorithm}
  ...
  \For{each $cert \in QC$}
  {
    \If{$cert.Issuer \in Clique$ {\bf and} $Verify(cert.Issuer,
      cert.Signature)$
    }{
      $Counter$++\;
    }
...
\newcounter{a}
\newcounter{b}
\foreach \p/\t/\l in {40/$H_1$/Honest nodes, 40/$H_2$/Honest nodes, 20/$F$/Faulty nodes}
  {
...
\draw[thick, domain=0:0.7] plot (\x, {1-pow(17/200,\x)});
\draw[thick, domain=0.7:1.0] plot (\x, {1-pow(17/200,0.7)});
\end{tikzpicture}
