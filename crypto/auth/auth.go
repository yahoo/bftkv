...
func (a *Auth) GeneratePartialAuthenticationData(cred []byte, n, k int) ([][]byte, error) {
	// generate a k-degree polynomial (on q)
	poly := make([]*big.Int, k)
	for i := 0; i < k; i++ {
	...
	for i := 1; i <= n; i++ {
		...
		params.x = i
		params.y = new(big.Int).Mod(t.Add(f, gpi), q)
		params.v = v
		params.salt = salt
		params.n = n
		params.k = k
...
func (a *Auth) MakeResponse(ss []byte, challenge []byte, plain []byte) (res []byte, cipherData []byte, err error) {
	...
	// Yi = X^y
	Yi := new(big.Int).Exp(X, params.y, p)
...
func (c *AuthClient) ProcessAuthResponse(res []byte, id uint64) (bool, error) {
	...
	gpi := new(big.Int).Exp(g, c.pi, q)

	inv := new(big.Int).ModInverse(c.a, q)		// t = a^-1 mod q
...
func (c *AuthClient) getSessionKey(id uint64) ([]byte, error) {
	...
	e.Mod(e.Add(c.a, e.Mod(e.Mul(u, x), phi)), phi)	// e=a+ux
